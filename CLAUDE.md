...
- Forced password change on first login using a generated one-time bootstrap password
- `must_change_password` flag on User model (enforced via `@app.before_request` hook, including APIs)
- Password complexity: 12+ chars, must have uppercase, lowercase, digit (`validate_password_strength` in `app/models/user.py`)
- Login rate limiting: 10 failures/IP or attempted email in 10-minute window, tracked in Redis (`login_fail:*` keys)
...
### User Flow
1. Login with bootstrap admin email and generated one-time password
...
# Login with the bootstrap admin email and one-time password printed by init_db.py
```
...
- Password: generated by init_db.py and printed once, or INITIAL_ADMIN_PASSWORD if provided