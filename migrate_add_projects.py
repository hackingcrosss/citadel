    ...
    with app.app_context():
        ...
        try:
            ...
            if table_exists('projects'):
                print("✓ Table 'projects' already exists — skipping create.")
            else:
                ...
                """))
            conn.execute(text("CREATE INDEX IF NOT EXISTS ix_projects_code ON projects (code)"))

            # --- Create project_members table ---
            if table_exists('project_members'):
                print("✓ Table 'project_members' already exists — skipping create.")
            else:
            ...
            if table_exists('project_resources'):
                print("✓ Table 'project_resources' already exists — skipping create.")
            else:
                ...
                """))
            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS ix_project_resources_project_id "
                "ON project_resources (project_id)"
            ))
            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS ix_project_resources_resource_type "
                "ON project_resources (resource_type)"
            ))
            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS ix_project_resources_external_id "
                "ON project_resources (external_id)"
            ))

            # --- Modify domains table: add checkout columns ---
            print("Checking checkout columns on 'domains' table...")
            if not column_exists('domains', 'checkout_project_id'):
                conn.execute(text(
                    "ALTER TABLE domains "
                    "ADD COLUMN checkout_project_id INTEGER REFERENCES projects(id)"
                ))
                print("  Added domains.checkout_project_id")
            else:
                print("  domains.checkout_project_id already exists — skipping.")

            if not column_exists('domains', 'checked_out_at'):
                conn.execute(text(
                    "ALTER TABLE domains "
                    "ADD COLUMN checked_out_at TIMESTAMP"
                ))
                print("  Added domains.checked_out_at")
            else:
                print("  domains.checked_out_at already exists — skipping.")

            if not column_exists('domains', 'checked_out_by_id'):
                conn.execute(text(
                    "ALTER TABLE domains "
                    "ADD COLUMN checked_out_by_id INTEGER REFERENCES users(id)"
                ))
                print("  Added domains.checked_out_by_id")
            else:
                print("  domains.checked_out_by_id already exists — skipping.")

            conn.execute(text(
                "CREATE INDEX IF NOT EXISTS ix_domains_checkout_project_id "
                "ON domains (checkout_project_id)"
            ))

            # --- Create default project owned by the first admin ---
            result = conn.execute(text(
                "SELECT id FROM projects WHERE code='DEFAULT' LIMIT 1"
            ))
            default_project = result.fetchone()
            if default_project:
                project_id = default_project[0]
                print(f"✓ Default project already exists (id={project_id}) — skipping create.")
            else:
